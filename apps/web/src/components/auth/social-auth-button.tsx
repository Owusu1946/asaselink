...
interface SocialAuthButtonProps {
  ...
  isLoading?: boolean;
  disabled?: boolean;
  onClick: (strategy: OAuthStrategy) => Promise<void> | void;
}
...
export function SocialAuthButton({
  ...
  const strategyMap: Record<"google" | "apple" | "facebook", OAuthStrategy> = {
    google: "oauth_google",
    apple: "oauth_apple",
  ...
  if (provider === "google") {
    return (
      <Button
        ...
        onClick={handleClick}
        className={cn(
          "w-full h-11 justify-center gap-3 border-border font-medium text-foreground bg-background hover:bg-secondary/70 transition-[color,background-color,border-color,box-shadow,opacity] duration-fast shadow-xs",
  ...
  if (provider === "apple") {
    return (
      <Button
        ...
        onClick={handleClick}
        className={cn(
          "w-full h-10 justify-center gap-2 border-border font-medium text-foreground bg-background hover:bg-secondary/70 transition-colors shadow-xs text-xs sm:text-sm",
          isLoading && "opacity-90 cursor-wait",
        )}
      >
        {isLoading ? (
        ...
        <span>{isLoading ? "Connecting..." : "Apple"}</span>
      </Button>
    );
  }
