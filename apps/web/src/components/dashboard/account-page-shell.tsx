...
const PAGE_META: Record<string, { title: string; description: string }> = {
  "/account": {
    ...
    description: "Manage verified land activity and documentation.",
  },
  "/account/reservations": {
    title: "My Reservations",
    description: "Active parcel reservations, escrow statuses, and deed preparation progress.",
  },
  "/account/payments": {
    title: "Payments",
    description: "Purchase payments and verification status.",
  },
  "/account/purchases": {
    title: "Purchases",
    description: "Track balances, installments, credits, refunds, and completion.",
  },
  "/account/saved": {
    title: "Saved Parcels",
    description: "Bookmarked plots, price alerts, and estate layouts saved for review.",
  },
  "/account/documents": {
    title: "Document Vault",
    description: "Official search reports, cadastral plans, and indenture deeds.",
  },
  "/account/alerts": {
    title: "Land Alerts",
    description: "Areas and budgets AsaseLink is actively watching for you.",
  },
};
...
export function AccountWorkspaceShell({ children }: { children: React.ReactNode }) {
  ...
  const meta =
    PAGE_META[pathname] ??
    (pathname.startsWith("/account/purchases/")
      ? PAGE_META["/account/purchases"]
      : PAGE_META["/account"])!;
        ...
        <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-border bg-background/90 px-4 backdrop-blur-md sm:px-8">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xs text-muted-foreground">
              <span className="font-medium text-foreground">Account</span> / {meta.title}
            </span>
          </div>
          <div className="hidden w-full max-w-sm px-6 md:block">
            <div className="relative">
              ...
              <input
                ...
                className="w-full rounded-xl border border-border bg-muted/50 py-1.5 pl-8 pr-3 text-xs outline-none focus:ring-1 focus:ring-ring"
              />
            </div>
          </div>
          <Link
            href="/"
            className={buttonVariants({ size: "sm", className: "h-8 gap-1.5 rounded-xl text-xs" })}
          >
