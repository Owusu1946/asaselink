...
export function EstateReviewAction({ estateId, status, plotCount }: { estateId: string; status: string; plotCount: number }) {
  ...
  return <div>{error ? <p role="alert" className="mb-3 text-sm text-destructive">{error}</p> : null}<Button type="button" disabled={plotCount === 0 || pending} onClick={() => startTransition(async () => { try { const result = await client.land.submitEstate({ estateId }); setCurrentStatus(String(result.status)); notify.success("Estate submitted", { description: "AsaseLink has received it for review." }); } catch (cause) { setError(cause instanceof Error ? cause.message : "The estate could not be submitted."); notify.apiError(cause, "Estate was not submitted"); } })} className="h-11">{pending ? "Submitting…" : "Submit estate for review"}</Button>{plotCount === 0 ? <p className="mt-2 text-xs text-muted-foreground">Add at least one mapped plot before submitting.</p> : null}</div>;
}
