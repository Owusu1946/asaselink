...
export function PlotEditor({ estateId, estateBoundary, plots, initialSitePlan, onPlotCreated }: Props) {
  ...
  function removeOverlay() {
    if (!sitePlan || !window.confirm("Remove this site plan overlay? Saved plot boundaries will not be affected.")) return;
    const planId = sitePlan.id;
    startTransition(async () => {
      try {
        await client.land.removeEstateSitePlan({ planId });
        const map = mapRef.current;
        if (map?.getLayer("site-plan-controls-line")) map.removeLayer("site-plan-controls-line");
        if (map?.getSource("site-plan-controls")) map.removeSource("site-plan-controls");
        if (map?.getLayer("site-plan-layer")) map.removeLayer("site-plan-layer");
        if (map?.getSource("site-plan")) map.removeSource("site-plan");
        overlayImageUrlRef.current = undefined;
        alignmentHistoryRef.current = [];
        setSitePlan(null);
        notify.success("Overlay removed", { description: "You can now trace plots directly on the satellite map." });
      } catch (cause) { notify.apiError(cause, "Overlay could not be removed"); }
    });
  }
