WITH recoverable AS MATERIALIZED (
  ...
  WHERE pa.status = 'PURCHASE_IN_PROGRESS'
    ...
    AND NOT EXISTS (
      ...
      WHERE other.plot_id = pa.plot_id AND other.id <> r.id
  ...
  FROM recoverable repair WHERE p.id = repair.plot_id RETURNING p.id
), restored AS (
  ...
  WHERE r.id = repair.reservation_id AND repair.plot_id IN (SELECT id FROM reserved)
...
SELECT 'reservation.purchase_restored', 'reservation', id::text,
  'Final repair after deploying corrected expiry worker', jsonb_build_object('plotId', plot_id)
FROM restored;
