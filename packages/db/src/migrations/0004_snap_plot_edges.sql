...
BEGIN
  ...
  IF NOT ST_CoveredBy(NEW.boundary, estate_boundary) THEN
    IF ST_CoveredBy(NEW.boundary, ST_Buffer(estate_boundary::geography, 1)::geometry) THEN
      ...
      IF GeometryType(clipped_boundary) <> 'POLYGON' OR ST_IsEmpty(clipped_boundary) THEN
  ...
  END IF;
  IF EXISTS (
    SELECT 1 FROM plots p WHERE p.estate_id = NEW.estate_id AND p.id <> NEW.id
  ...
  END IF;
  IF EXISTS (
    SELECT 1 FROM restricted_areas r WHERE r.estate_id = NEW.estate_id
