...
export const purchaseAccounts = pgTable(
  "purchase_accounts",
  {
    ...
    sourceReservationId: uuid("source_reservation_id")
      .notNull()
  ...
  (table) => [
    ...
    uniqueIndex("purchase_accounts_one_live_plot_uq")
      .on(table.plotId)
      .where(sql`${table.status} in ('HOLD_ACTIVE','PURCHASE_IN_PROGRESS','PAID')`),
    ...
    check(
      "purchase_accounts_status_check",
      sql`${table.status} in ('HOLD_ACTIVE','PURCHASE_IN_PROGRESS','PAID','COMPLETED','CANCELLED','REFUND_PENDING')`,
    ),
...
export const purchaseLedgerEntries = pgTable(
  "purchase_ledger_entries",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    purchaseAccountId: uuid("purchase_account_id")
      .notNull()
      .references(() => purchaseAccounts.id, { onDelete: "restrict" }),
  ...
  (table) => [
    ...
    uniqueIndex("purchase_ledger_payment_type_uq")
      .on(table.paymentId, table.type)
      .where(sql`${table.paymentId} is not null`),
    uniqueIndex("purchase_ledger_hold_credit_uq")
      .on(table.purchaseAccountId, table.type)
      .where(sql`${table.type} = 'HOLD_CREDIT' and ${table.status} = 'CONFIRMED'`),
    uniqueIndex("purchase_ledger_reversal_uq")
    ...
    check(
      "purchase_ledger_type_check",
      sql`${table.type} in ('HOLD_CREDIT','DEPOSIT','INSTALLMENT','BALANCE','FINAL_PAYMENT','REFUND','ADJUSTMENT','REVERSAL')`,
    ),
