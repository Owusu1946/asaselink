...
export const payments = pgTable(
  "payments",
  {
    ...
    platformFeeAmount: numeric("platform_fee_amount", { precision: 14, scale: 2 })
      .notNull()
      .default("0"),
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.method} in ('MTN_MOMO','TELECEL_CASH','AIRTELTIGO_MONEY','BANK_TRANSFER')`,
    ),
    check(
    ...
    check(
      ...
      sql`${table.purpose} in ('PURCHASE','HOLD_FEE','DEPOSIT','INSTALLMENT','BALANCE','FINAL_PAYMENT')`,
    ),
    check(
      "payments_amount_check",
      sql`${table.amount} > 0 and ${table.platformFeeAmount} >= 0 and ${table.developerNetAmount} >= 0 and ${table.amount} = ${table.platformFeeAmount} + ${table.developerNetAmount}`,
    ),
...
export const payoutRequests = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.status} in ('REQUESTED','APPROVED','PROCESSING','PAID','REJECTED','CANCELLED')`,
    ),
    check(
      "payout_requests_destination_check",
      sql`${table.destinationType} in ('MOBILE_MONEY','BANK_ACCOUNT')`,
    ),
  ],
);

export const companyLedgerEntries = pgTable(
  ...
  (table) => [
    ...
    check(
      "company_ledger_type_check",
      sql`${table.type} in ('SALE_CREDIT','PAYOUT_DEBIT','REFUND_DEBIT','ADJUSTMENT_CREDIT','ADJUSTMENT_DEBIT')`,
    ),
    check(
      "company_ledger_status_check",
      sql`${table.status} in ('PENDING','AVAILABLE','SETTLED','REVERSED')`,
    ),
    check("company_ledger_amount_check", sql`${table.amount} > 0`),
    check("company_ledger_currency_check", sql`${table.currency} = 'GHS'`),
    check(
      "company_ledger_source_check",
      sql`(${table.paymentId} is not null)::int + (${table.payoutRequestId} is not null)::int = 1`,
    ),
  ],
);
