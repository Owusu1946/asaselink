...
function configuration() {
  ...
  if (Object.values(values).some((value) => !value))
    throw new Error(
      "Document storage is not configured. Add the R2 credentials to the API environment.",
    );
...
export function paymentProofKey(buyerId: string, paymentId: string, fileName: string) {
  return `buyers/${buyerId}/payment-proofs/${paymentId}/${crypto.randomUUID()}-${safeFileName(fileName)}`;
}
