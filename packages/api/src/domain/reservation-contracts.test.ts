...
const migration = readFileSync(resolve(root, "packages/db/src/migrations/0014_daily_sway.sql"), "utf8");
...
describe("paid hold persistence contracts", () => {
  it("backfills legacy outcomes without replacing buyer, plot, price, or expiry", () => {
    ...
    const backfill = migration.match(/UPDATE "reservations"[\s\S]*?statement-breakpoint/)?.[0] ?? "";
  ...
  it("retains database protection against two live claims on one plot", () => {
    ...
    expect(migration).toContain("'CHECKOUT_LOCKED','HOLD_PAYMENT_PENDING','HELD','PURCHASE_IN_PROGRESS'");
  });
  ...
  it("makes overlapping expiry runs safe and refund creation idempotent", () => {
    ...
    expect(jobs).toContain("status IN ('CHECKOUT_LOCKED','HOLD_PAYMENT_PENDING','HELD','PURCHASE_IN_PROGRESS')");
  });
});
